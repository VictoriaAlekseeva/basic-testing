...
describe('BankAccount', () => {
  ...
  test('should throw InsufficientFundsError error when withdrawing more than balance', () => {
    expect(() => account.withdraw(1001)).toThrowError(`Insufficient funds: cannot withdraw more than ${initialBalance}`);
  });

  test('should throw error when transferring more than balance', () => {
    const account2 = getBankAccount(200);
    expect(() => account.transfer(1001, account2)).toThrowError(`Insufficient funds: cannot withdraw more than ${initialBalance}`);
  });

  test('should throw error when transferring to the same account', () => {
    expect(() => account.transfer(999, account)).toThrowError(`Transfer failed`);
  });
